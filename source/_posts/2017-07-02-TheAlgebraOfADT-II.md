...
我们可以 [问一问 Wolfram Alpha](http://www.wolframalpha.com/input/?i=Series%5B+1%2F%281-a%29+%5D+at+a+%3D+0) *1 /* (*1 - a*) 的泰勒级数是什么，然后它会告诉我们：
...
其中 *q1* 和 *q2* 不是常量（他们不能只是 1 ），那么同样的结果对于半环也是正确的。特别地，它对于类型也是正确的，也就是你可以找到一个不使用减法以及其它对于类型而言非法的运算的证明。
